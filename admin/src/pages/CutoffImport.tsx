...
export default function CutoffImport() {
  ...
  const handleImport = async () => {
    ...
    setError('');
    try {
      const res = await importCutoffs(file);
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 'var(--space-5)' }}>
        <button onClick={() => navigate('/cutoffs')} style={backBtnStyle}>
          <ArrowLeft size={16} />
        </button>
        <h2 style={{ fontSize: 16, fontWeight: 600 }}>Import Cutoffs from CSV</h2>
      </div>
      ...
      <div style={{ maxWidth: 640 }}>
        ...
        {!result && file && (
          <button onClick={handleImport} disabled={loading} style={{ ...importBtnStyle, opacity: loading ? 0.6 : 1 }}>
            {loading ? 'Importing...' : 'Import Cutoffs'}
          </button>
        )}
        ...
        {result && (
          <div>
            <div style={{
              ...
              <div>
                <div style={{ fontWeight: 600, fontSize: 14 }}>Imported {result.imported} cutoff{result.imported !== 1 ? 's' : ''}</div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => navigate('/cutoffs')} style={primaryBtnStyle}>View Cutoffs</button>
            </div>
          </div>
        )}
      </div>
