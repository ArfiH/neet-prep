...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    html, body { width: 100%; height: auto; min-height: 100%; overflow: auto; background: #1a1a1a; font-family: -apple-system, BlinkMacSystemFont, sans-serif; -webkit-tap-highlight-color: transparent; }
    #pdf-container { width: 100%; min-height: 100vh; display: flex; flex-direction: column; align-items: center; overflow-y: visible; overflow-x: hidden; padding: 20px; padding-bottom: 60px; -webkit-overflow-scrolling: touch; touch-action: manipulation; }
    .page-wrapper { position: relative; display: inline-block; width: 100%; margin-bottom: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.5); -webkit-touch-callout: none; }
    .page-wrapper canvas { display: block; width: 100%; height: auto; }
    .watermark { position: absolute; top: 0; left: 0; right: 0; bottom: 0; display: flex; justify-content: center; align-items: center; pointer-events: none; z-index: 10; }
    .watermark-text { font-size: 20px; font-weight: 800; color: rgba(0, 0, 0, 0.1); white-space: nowrap; }
    #loading { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); color: white; font-size: 16px; text-align: center; }
    .spinner { width: 40px; height: 40px; border: 4px solid rgba(255,255,255,0.2); border-top-color: white; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 16px; }
    @keyframes spin { to { transform: rotate(360deg); } }
    #error-message { display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); color: #ff6b6b; font-size: 16px; text-align: center; padding: 20px; }
  </style>
</head>
<body>
  <div id="pdf-container">
    <div id="loading"><div class="spinner"></div><p>Loading PDF...</p></div>
  ...
  <script>
    ...
    document.addEventListener('keydown', function(e) { if (e.ctrlKey || e.metaKey) { if (e.key !== '+' && e.key !== '-' && e.key !== '=' && e.key !== '0') { e.preventDefault(); return false; } } });
    ...
    pdfjsLib.getDocument(pdfUrl).promise.then(function(pdf) {
      ...
      function renderPage(pageNum) {
        ...
        pdf.getPage(pageNum).then(function(page) {
          ...
          page.render({ canvasContext: context, viewport: viewport }).promise.then(function() { renderPage(pageNum + 1); });
        });
      }
      renderPage(1);
    }).catch(function(error) { loading.style.display = 'none'; errorMsg.style.display = 'block'; console.log('PDF load error:', error); });
  </script>
</body>
</html>
`;
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.webviewContainer}>
          <WebView
            ...
            onLoadEnd={() => console.log('PDF loaded via PDF.js')}
            renderLoading={() => (
...
const styles = StyleSheet.create({
  ...
  backBtn: { position: 'absolute', top: 50, left: 16, zIndex: 10, backgroundColor: COLORS.stage, width: 28, height: 28, borderRadius: 14, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: COLORS.border },
  titleContainer: { paddingTop: 32, paddingBottom: 10, paddingHorizontal: 16, backgroundColor: COLORS.surface, borderBottomWidth: 1, borderBottomColor: COLORS.border },
  title: { fontSize: 14, fontWeight: '600', color: COLORS.fg, textAlign: 'center' },
  ...
  webviewContainer: { flex: 1, backgroundColor: '#1a1a1a' },
  webview: { flex: 1, width, height },
  loadingOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.9)' },
});
